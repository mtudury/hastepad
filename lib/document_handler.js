...
const checkkeyregex = /^[a-zA-Z0-9_][a-zA-Z0-9-_.]{0,80}$/;
...
var DocumentHandler = function(options) {
  ...
  this.keyDefaultExtension = options.keyDefaultExtension || '';
};
...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var onSuccess = function () {
    ...
    const store = function (key) {
       _this.store.set(key, buffer, function (res, created) {
        if (res) {
...
DocumentHandler.prototype.handleList = function(request, response, config) {
  ...
  this.store.list(function(ret) {
      ...
      // filter files
      var filteredlist = [];
      for (const fil of ret) {
        if (checkkeyregex.test(fil)) {
          filteredlist.push(fil);
        }
      }
...
DocumentHandler.prototype.acceptableKey = function() {
  return this.keyGenerator.createKey(this.keyLength) + this.keyDefaultExtension;
};
